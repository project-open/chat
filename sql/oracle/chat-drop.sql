...
begin
    for v_rec in (select object_id from acs_objects where object_type = 'chat_transcript')
    loop
...
begin
    for v_rec in (select object_id from acs_objects where object_type = 'chat_room')
    loop
        acs_object.del(v_rec.object_id);
    end loop;
end;
